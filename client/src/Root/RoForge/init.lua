...
function RoForge.start(plugin)
	...
	local okTb, toolbar = pcall(function()
		return plugin:CreateToolbar("RoForge")
	end)
	if okTb and toolbar then
		local okB, btn = pcall(function()
			...
			return toolbar:CreateButton("RoForge", "Open or close the RoForge chat", "", "RoForge")
		end)
		if not okB then
			okB, btn = pcall(function()
				return toolbar:CreateButton("RoForge", "Open or close the RoForge chat")
			end)
		end
		if okB and btn then
			toggle = btn
		else
			warn("\[RoForge] could not create toolbar button (%s); if you close the dock, re-open it from Studio's Widgets menu." % tostring(btn))
		end
	end
	if toggle then
		...
		toggle.Click:Connect(function()
			local visible = not dock.Enabled
